...
In the following chart it can be seen the impact of customer and service reviews on sales volume. It is observed that when more _Positive Service Reviews_ and _4 Star Reviews_ there are, the sales volume is higher. 

```{r, echo=FALSE}
EPA2 <-read.csv("C:/Users/Burbu/Documents/Projects/Ubiqum/Task3/data/existingproductattributes2017.csv")
# Remove outliers from the dependent variable
OutlierDataSet <- EPA2
OutlierColumn <- OutlierDataSet[,ncol(OutlierDataSet)]
OutlierDataSet <- OutlierDataSet[OutlierColumn > (quantile(OutlierColumn)[[2]] - 1.5*IQR(OutlierColumn)),]
...
* According to the model (_Random Forest, RMSE = 118, R-squared = 0,96, MAE =74_), it can be said that it is predicted to sale ¿1700? Netbooks, X Smartphones, X Laptops and X PCs. 
* It has been found that the variables which better predict the _Sales Volume_, according our model, are _4Star Reviews_ and _Total Service Reviews_ (a created feature). 
* It can be observed that customers and service variables are strongly related with sales volume, being two of them the better predictors. 
* It is recommended to sales department take these predictions carefully. It has been found a lot of problems (small sample, outliers, duplicated values, etc.) in our data set which could affected the model, the results and the predictions. It is required to earn more new data for review this analysis. 
